...
train_starting_velocity1 = np.random.uniform(0, max_angle/3, (n_train))
train_starting_velocity2 = np.random.uniform(0, max_angle/3, (n_train))
...
test_starting_velocity1 = np.random.uniform(-max_angle/3, max_angle/3)
test_starting_velocity2 = np.random.uniform(-max_angle/3, max_angle/3)
...
#    print("current jitter %s" %jitter)
    ...
    for invar_density in [5]: #np.arange(10, 30, 10):
            try:
                kernel = get_Polynomial_2D_Local_Invariance(2, 10, jitter, [1,1,1,1]) #switch
