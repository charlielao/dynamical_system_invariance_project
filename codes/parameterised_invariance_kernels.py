...
class Spline_Invariance(gpflow.kernels.Kernel):
    def __init__(self, invariance_range, invar_density, jitter_size, spline_f_n_basis, spline_g_n_basis,spline_f_degree, spline_g_degree, knots_range, knots_density):
        super().__init__(active_dims=[0, 1])
        self.spline_f_n_basis = spline_f_n_basis
        self.spline_g_n_basis = spline_g_n_basis
        self.spline_f_degree = spline_f_degree
        self.spline_g_degree = spline_g_degree
        self.knots = tf.linspace(-knots_range, knots_range, knots_density)
        self.f_spline = gpflow.Parameter(tf.Variable(0.5*tf.random.normal((self.spline_f_n_basis, 1), dtype=tf.float64)), transform =tfp.bijectors.Sigmoid(to_default_float(-2.), to_default_float(2.)))
        self.g_spline = gpflow.Parameter(tf.Variable(0.5*tf.random.normal((self.spline_g_n_basis, 1), dtype=tf.float64)), transform =tfp.bijectors.Sigmoid(to_default_float(-2.), to_default_float(2.)))
        self.jitter =jitter_size
        self.Ka = gpflow.kernels.RBF(variance=1, lengthscales=[1,1]) 
        self.Kv = gpflow.kernels.RBF(variance=1, lengthscales=[1,1]) 
        invariance_xs = tf.linspace(-invariance_range,invariance_range,invar_density)
    ...
    def inv_f(self, X):
        splines = interpolate.BSpline(self.knots, self.f_spline, self.spline_f_degree)
        return tf.linalg.matmul(tf.math.pow(X, list(range(self.poly_f_d))), self.f_poly)
    def inv_g(self, X):
        return tf.linalg.matmul(tf.math.pow(X, list(range(self.poly_g_d))), self.g_poly)
