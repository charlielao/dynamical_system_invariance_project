...
class PolynomialInvariance2D(gpflow.kernels.Kernel):
    def __init__(self, invariance_range, invar_density, jitter_size, poly_d):
        ...
        self.f1_poly = gpflow.Parameter(tf.Variable(1e-8*tf.ones((self.number_of_coefficients(self.poly_d[0]),1), dtype=tf.float64)), transform =tfp.bijectors.Sigmoid(to_default_float(-1.), to_default_float(1.)))#, prior=tfp.distributions.Laplace(to_default_float(0),to_default_float(self.prior_variance)))
        self.f2_poly = gpflow.Parameter(tf.Variable(1e-8*tf.ones((self.number_of_coefficients(self.poly_d[1]),1), dtype=tf.float64)), transform =tfp.bijectors.Sigmoid(to_default_float(-1.), to_default_float(1.)))#, prior=tfp.distributions.Laplace(to_default_float(0),to_default_float(self.prior_variance)))
        self.g1_poly = gpflow.Parameter(tf.Variable(1e-8*tf.ones((self.number_of_coefficients(self.poly_d[2]),1), dtype=tf.float64)), transform =tfp.bijectors.Sigmoid(to_default_float(-1.), to_default_float(1.)))#, prior=tfp.distributions.Laplace(to_default_float(0),to_default_float(self.prior_variance)))
        self.g2_poly = gpflow.Parameter(tf.Variable(1e-8*tf.ones((self.number_of_coefficients(self.poly_d[3]),1), dtype=tf.float64)), transform =tfp.bijectors.Sigmoid(to_default_float(-1.), to_default_float(1.)))#, prior=tfp.distributions.Laplace(to_default_float(0),to_default_float(self.prior_variance)))
