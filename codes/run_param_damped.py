...
for gamma in [0.01, 0.05, 0.1]:
    ...
    data = get_damped_pendulum_data(gamma, 0.5, 0.1, [90, 130]) #switch
        ...
        for invar_density in [20]:#np.arange(10, 40, 10):
            for poly_f_d in [5, 6, 7]:
                for poly_g_d in [5, 6, 7]:
                    try:
                        kernel = get_Damped_Polynomial_Local_Invariance(0.1, jitter, poly_f_d, poly_g_d)#switch
                        m, pred, var = get_GPR_model(kernel, zero_mean(2), data, test_grids, 300)
