...
\begin{frame}
	...
	\only<1>{\begin{definition}\begin{quote}GP is a collection of random variables, any finite number of which have a joint Gaussian distribution (\cite{rasmussen_williams_2006})\end{quote}
	\end{definition}}
\only<2-4>{
$$
\begin{bmatrix}
...
\caption{Comparing the capabilities of different existing approach to learning invariance in dynamical systems}
    \end{table}
  \cite{Raissi2019}, \cite{HNN}, \cite{NeuralODE}, \cite{GraphSymbolicPhysics}
\end{frame}
...
\end{pmatrix}$$}
\onslide<3->{
$$K(X, X')=\mathrm{Cov}(\mathbf{f}(X), \mathbf{f}(X'))=
$$
$$
\begin{pmatrix}
...
\onslide<1->{We had $L[\mathbf{f}(X_L)]=\frac{dE}{dt}=0,$ but in a damped system $\frac{dE}{dt}<0$\newline}
\onslide<2->{Now instead, we have $L'[\mathbf{f}(X_L)]=L+\epsilon$, where $\epsilon\sim\mathcal{N}(m_L, \sigma_L^2)$}
\onslide<3->{
$$
\begin{pmatrix}
\mathbf{f}(X)\\L'[\mathbf{f}(X_L)]
...
\onslide<4->{
  $$\mathbf{f}(X)|L'[\mathbf{f}(X_L)]=0 \sim \mathcal{N} (-LK(LKL^T+\sigma^2_L\mathbb{I})^{-1}\mathbf{m}_\ell,$$
  $$ \begin{pmatrix}
...
\begin{frame}{Damped System- Latent Dynamics}
	\onslide<1->{
Now to model the missing part that makes an invariant system no longer invariant, we invent a latent variable $z$ such that
$$
...
\onslide<3->{Extensions of latent variable models}
...
\begin{frame}{Experiments}
	\begin{enumerate}
		...
		\item Implementation Technicalities 
	\end{enumerate}
\end{frame}
...
D_{ij} &= \mathrm{Cov}(L[\mathbf{f}(X_L)], L[\mathbf{f}(X_L)])_{ij} \\
       &= \mathrm{Cov}(mp_{L,i}a(q_{L,i},p_{L,i}) + kq_{L,i}v(q_{L,i},p_{L,i}), \\
       &mp_{L,i}a(q_{L,i},p_{L,i}) + kq_{L,i}v(q_{L,i},p_{L,i})) \\
...
\begin{frame}{Invariance Priors}
\only<1>{
\begin{figure}[H]
     \centering
         \includegraphics[width=0.8\linewidth]{../codes/figures/prior_shm_invariance.pdf}
         \caption{SHM invariance prior}
\end{figure}}
\only<2>{
\begin{figure}[H]
     \centering
         \includegraphics[width=0.8\linewidth]{../codes/figures/prior_pendulum_invariance.pdf}
...
\begin{itemize}
  \item  $E=\frac{mp^2}{2}+\frac{kq^2}{2}$, $\frac{dE}{dt}=mpa+kqv$.
  \item  $p=v=\frac{dq}{dt}\Rightarrow\frac{dE}{dt}=mva+kvq=v(ma+kq)$
  \item  $\frac{d^2q}{dt^2}+2\gamma \frac{dq}{dt} +\frac{kq}{m} =0 \equiv m\frac{dp}{dt}+2m\gamma v + kq=0$
  \item $ma+2m\gamma v+kq=0$ or $ma+kq=-2m\gamma v\equiv -bv$.
  \item $\frac{dE}{dt} = v(-bv) = -bv^2$
  \item $\frac{dE}{dt}+z=0 \Rightarrow z=bv^2=bp^2$
\end{itemize}
	}
