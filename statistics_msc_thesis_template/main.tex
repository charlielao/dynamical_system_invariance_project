...
If we have the ODE $\frac{dx}{dt}=f$; then $$\mathcal{L}[L]=\frac{dL}{dt}=f(x)\cdot \nabla_x L(x),$$ where $L$  is some function, following by simple chain rule.
If we would like $L$ to be the invariance, we would then require $\mathcal{L}[L]=0$. 
