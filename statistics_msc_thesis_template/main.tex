...
\subsection{GPflow}
While we can in principle build our own GP objects using linear algebra and statistical libraries, such as Numpy, Scipy, from scratch, it is better to avoid errors and improve development speed by using a well-tested, robust and efficient library called GPflow, which will be used throughout the project (\cite{GPflow2017}).
...
Refering to \cite{Kondor2008}, we have symmetries being invariant if we have a group $G$ acting on $\mathcal{X}$ by $x\mapsto T_g(x)$ for $g\in G$, where $T$ is some sort of transformation function. 
Then we should have $f(T_g(x))=f(x)$ for any function $f$.
For instance, if it is a rotational symmetric group then we will have $T$ corresponds to the angle of rotation, and we expect $f$ to be invariant under rotation.
In dynamical system, it has a slightly different interpretation.
From \cite{marsden_sirovich_antman_2008}, in this description of dynamical system, we consider a generator $\mathcal{L}$, which generates the evolution of some function quantitiy that depends on the input over time.
If we have the ODE $\frac{dx}{dt}=f$; then $$\mathcal{L}[E]=\frac{dE}{dt}=f(x)\cdot \nabla_x E(x),$$ where $E$  is some function, following by simple chain rule.
...
In physics, there is a deeper connection between symmetry and invariance, namely the Neother's Theorem, which is rather involved in theoretical classical physics and interested readers can refer to \cite{lemos_2018}.
...
We can see the invariance constraints will always be a linear function on the dynamics $\mathbf{f}=\begin{pmatrix}
  \mathbf{a} \\ \mathbf{v}
\end{pmatrix}$ almost by definition because of chain rule.
We have $\text{energy}=E(\theta, \mathbf{p}, \mathbf{q})$, where $\theta$ are constants that parameterised the energy function, like mass or gravity.
Then we have 
$$
...
As a result, $\mathcal{L}[E]$ can be seen to be a linear transformation of $\mathbf{f}$, and \textbf{now we call this transformation operator $L$}, which will be a function of $p, q$ and $\theta$.
...
Previously, we have assumed the knowledge of the dynamics equation from Newton's Law, which allowed us to derive the energy, $E$ and thereafter the invariance equation $L$ accordingly. 
...
In 1D cases, for our examples, they are simple enough so that $L(p,q)$ is seperable so that we have a function of $p$ only, $f(p)$ and a different function of $q$ only, $g(q)$ so that they can be learnt seperately.
In the 1D SHM case, $f(p)=mp$ and $g(q)=kq$.
...
In 2D, things are a bit more complicated. 
...
Therefore, for 2D we have $L(p_1, p_2, q_1, q_2)$ being the combination of $f_1(p_1), f_2(p_2), g_1(q_1), g_2(q_2)$.
...
\begin{figure}[H] 
  \includegraphics[width=0.8\linewidth]{../codes/figures/shm_trajectory_1D.pdf}
  \centering
...
\begin{figure}[H]
        \centering
        \includegraphics[width=0.4\linewidth]{../figures/pendulum.pdf}
...
We see that with invariance kernel, it is at least ten times more data efficient than the baseline.
...
\subsection{Linear SHM}
A simple extension to 1D SHM to 2D is just allowing the spring to be at an angle so that the system is now in a 2D space as shown in figure \ref{fig:shm_2d_diagram}, and the system is still constrained in the plane.
...
For implementation, we again use the same set of input space as the 1D space, but double the dimensions. 
...
In this work, we have presented a new Gaussian Process model through an invariance kernel that is able to capture the invariance in a dynamical system and as a result, use it as an inductive bias to improve data efficiency and predictive performance in a wide range of one and two-dimensional physical systems. 
We construct this special invariance kernel using simple Schur Complement calculation that conditions our GP prior on the points where the invariance should holds.
...
We have also demonstrated it is also robust against dissiaptive effects. 
