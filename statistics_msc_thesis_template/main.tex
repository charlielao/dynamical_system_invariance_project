...
For example, we wouldn't expect a lengthscale of 100 in our example with maximum range of 5; therefore, we will set for example, the maximum range of search is $10^{-3}$ to $10$ for the length scale and similarly for the variance term.
...
The performance is summariesed in table 
\subsection{Pendulum}
...
The rest is the same and we will do a test from $120^{\circ} to 160^{\circ}$ too.
\subsection{Damped SHM}
The results is the same, and the learnt $\epsilon$ is summarised in table. 
