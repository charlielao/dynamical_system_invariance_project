...
class SHO_Energy_Invariance_unknown_parameter(gpflow.kernels.Kernel):
    def __init__(self, invariance_range, invar_density):
        super().__init__(active_dims=[0, 1])
        self.weight_f_0 = gpflow.Parameter(tf.reshape(1.,(1,1)),transform =tfp.bijectors.Sigmoid(to_default_float(0.1), to_default_float(5.)) )
        self.weight_g_0 = gpflow.Parameter(tf.reshape(4.,(1,1)),transform =tfp.bijectors.Sigmoid(to_default_float(0.1), to_default_float(5.)) )
